...
export function InfluencerPerformanceAnalysis({ data }: InfluencerPerformanceAnalysisProps) {
  ...
  // 인플루언서 목록 (검색 기능 포함)
  const influencers = Array.from(new Set(data.map(item => item.username)))
    .filter(username => username.toLowerCase().includes(searchTerm.toLowerCase()))
    .sort();

  // 선택된 인플루언서의 데이터
  const selectedInfluencerData = data.filter(item => item.username === selectedInfluencer);
  
  // 선택된 인플루언서의 기본 정보
  const influencerInfo = selectedInfluencerData[0];

  // 일별 성과 데이터 생성 (실제 데이터를 기반으로 일별 변화를 시뮬레이션)
  const generateDailyData = (baseData: Influencer, days: number): DailyPerformanceData[] => {
    ...
    for (let i = 0; i < days; i++) {
      ...
      dailyData.push({
        ...
        views: Math.round((baseData.views || 0) * daysFactor * dailyVariation),
        saves: Math.round((baseData.saves || 0) * daysFactor * dailyVariation),
        comments: Math.round((baseData.comments || 0) * daysFactor * dailyVariation),
        likes: Math.round((baseData.likes || 0) * daysFactor * dailyVariation),
        shares: Math.round((baseData.shares || 0) * daysFactor * dailyVariation),
        responseRate: Math.round((baseData.responseRate || 0) * responseVariation * 10) / 10,
        followerCount: Math.round((baseData.followerCount || 0) * (1 + (i / days) * 0.05)) // 팔로워는 천천히 증가
      });
    }
    return dailyData;
  };
  ...
  const calculateMetrics = (data: typeof dailyPerformanceData) => {
    ...
    return {
      ...
      totalSaves: data.reduce((sum, item) => sum + (item.saves || 0), 0),
      totalComments: data.reduce((sum, item) => sum + (item.comments || 0), 0),
      totalLikes: data.reduce((sum, item) => sum + (item.likes || 0), 0),
      totalShares: data.reduce((sum, item) => sum + (item.shares || 0), 0),
      avgResponseRate: data.reduce((sum, item) => sum + (item.responseRate || 0), 0) / data.length,
      viewsChange: previous ? (((latest.views || 0) - (previous.views || 0)) / (previous.views || 1) * 100) : 0,
      savesChange: previous ? (((latest.saves || 0) - (previous.saves || 0)) / (previous.saves || 1) * 100) : 0,
      responseRateChange: previous ? (((latest.responseRate || 0) - (previous.responseRate || 0)) / (previous.responseRate || 1) * 100) : 0
    };
  };
            ...
            <div className="max-h-96 overflow-y-auto">
              {influencers.length === 0 ? (
                ...
                <div className="divide-y divide-slate-100">
                  {influencers.slice(0, 50).map(influencerUsername => {
                    const influencerData = data.find(item => item.username === influencerUsername);
                    if (!influencerData) return null;
                    
                    const isSelected = selectedInfluencer === influencerUsername;

                    return (
                      <div
                        key={influencerUsername}
                        onClick={() => setSelectedInfluencer(influencerUsername)}
                            ...
                            <div className="flex items-center space-x-4 text-sm">
                              <div className="text-center">
                                ...
                                <p className="font-medium text-slate-900">
                                  {((influencerData.followerCount || 0) / 1000).toFixed(0)}K
                                </p>
                              </div>
                              <div className="text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                <div className="lg:col-span-2">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-slate-900">{influencerInfo.username}</h3>
                ...
                <div className="text-center p-3 bg-blue-50 rounded-lg">
                  ...
                  {dailyPerformanceData.length > 1 && (
                    <p className="text-xs text-green-600">
                      +{(latestFollowerCount - (influencerInfo.followerCount || 0)).toLocaleString()}
                    </p>
                  )}
                </div>
                ...
                <div className="text-center p-3 bg-amber-50 rounded-lg">
                  ...
                  <p className="text-lg font-semibold text-amber-600">
                    {influencerInfo.date ? new Date(influencerInfo.date).toLocaleDateString('ko-KR', {
                      month: 'short',
                      day: 'numeric'
                    }) : '-'}
                  </p>
