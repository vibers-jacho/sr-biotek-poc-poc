...
export function VisualizationBoard({ data }: VisualizationBoardProps) {
  ...
  // 캠페인별 데이터 처리
  const processCampaignData = (inputData: Influencer[], metric: 'responseRate' | 'views' | 'saves' | 'emv' | 'roas') => {
    const campaignData = inputData.reduce((acc, item) => {
      if (!item.campaign || item[metric] === undefined || item[metric] === null) return acc;
  ...
  // 국가별 데이터 처리
  const processCountryData = (metric: 'responseRate' | 'views' | 'emv' | 'roas', inputData: Influencer[]) => {
  ...
  const processFollowerTypeData = (metric: 'responseRate' | 'views' | 'emv' | 'roas', inputData: Influencer[]) => {
    return inputData.reduce((acc, item) => {
  ...
  // 기간별 통계 계산 함수
  const calculatePeriodStats = (periodData: Influencer[]) => {
    ...
    return {
      ...
      totalViews: periodData.reduce((sum, item) => sum + (item.views || 0), 0),
      totalSaves: periodData.reduce((sum, item) => sum + (item.saves || 0), 0),
      averageResponseRate: Number((periodData.reduce((sum, item) => sum + (item.responseRate || 0), 0) / periodData.length).toFixed(1)),
      totalEMV: Number(periodData.reduce((sum, item) => sum + (item.emv || 0), 0).toFixed(1)),
      averageROAS: Number((periodData.reduce((sum, item) => sum + (item.roas || 0), 0) / periodData.length).toFixed(1)),
      totalCost: Number(periodData.reduce((sum, item) => sum + (item.cost || 0), 0).toFixed(1))
    };
  };
